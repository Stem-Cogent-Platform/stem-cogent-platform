...
export default function CompanyPage() {
  ...
  return <WorkspaceShell><section className="content-page"><div className="page-heading"><div><p className="eyebrow">Shared organisational view</p><h1>Company Lens</h1></div><Link className="secondary-button" href="/briefing">Switch to My Lens</Link></div>
    ...
    {state.status === "ready" && <><section className="company-summary"><div><small>Markets</small><strong>{state.data.profile?.operating_markets.join(", ") || "Context incomplete"}</strong></div><div><small>Strategic priorities</small><strong>{state.data.profile?.strategic_priorities.join(", ") || "Not configured"}</strong></div><div><small>Open material briefs</small><strong>{state.data.briefs.length}</strong></div></section><div className="card-list">{state.data.briefs.length ? state.data.briefs.map((brief) => <BriefCard brief={brief} key={brief.id} />) : <section className="empty-brief"><h2>No company-level Decision Briefs are open.</h2><p>The Company Briefs query completed successfully with zero open results. Stem will surface a brief when stored evidence and Company Context meet the materiality threshold.</p></section>}</div></>}
