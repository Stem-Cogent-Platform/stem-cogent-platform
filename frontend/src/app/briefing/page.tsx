...
type BriefingData = {
  ...
  readiness: { state: string; message?: string | null };
};
...
export default function BriefingPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [monitoring, changes, readiness] = capabilities.phase5_new_ui_enabled
        ? await Promise.all([
            apiRequest<MonitoringItem[]>("/api/v1/relevant-monitoring?limit=8"),
            apiRequest<BriefingData["changes"]>("/api/v1/briefing/changes"),
            apiRequest<BriefingData["readiness"]>("/api/v1/briefing/readiness"),
          ])
        : [[], { new_briefs: 0, updated_briefs: 0, new_evidence_items: 0, new_relevant_monitoring: 0 }, { state: "LEGACY" }];
