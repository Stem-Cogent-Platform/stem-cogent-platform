...
type MonitoringItem = {
  ...
  relevance_trace?: { why_relevant?: string };
  published_at?: string;
  detected_at: string;
...
type BriefingData = {
  ...
  changes: {
    new_briefs: number;
    updated_briefs: number;
    new_evidence_items: number;
    new_relevant_monitoring: number;
    as_of?: string;
    since_known?: boolean;
    new?: number;
    updated?: number;
    ...
    new_evidence?: number;
    new_decision?: number;
    system_freshness_at?: string | null;
    content_freshness_at?: string | null;
  };
...
export default function BriefingPage() {
  ...
  return <WorkspaceShell>
    ...
    {prepared && <section className="briefing-v2"><div className="briefing-main">
      <section aria-labelledby="attention-title"><div className="section-heading"><div><p className="eyebrow">Requires your attention</p><h2 id="attention-title">Material decisions</h2></div><span>{data.briefs.length} open</span></div><div className="card-list">{data.briefs.map((brief) => <BriefCard brief={brief} key={brief.id} />)}</div>{!data.briefs.length && <article className="empty-brief"><h3>{stateMessages.briefingEmpty.title}</h3><p>{data.readiness.state === "ASSESSED" ? stateMessages.briefingEmpty.body : "Stem is monitoring your configured scope. No verified recent development currently matches strongly enough to require action."}</p><Link className="secondary-button" href="/intelligence">Explore Intelligence</Link></article>}</section>
      <section aria-labelledby="monitoring-title" className="monitoring-section"><div className="section-heading"><div><p className="eyebrow">Relevant monitoring</p><h2 id="monitoring-title">Below the decision threshold</h2></div><Link href="/intelligence">Intelligence -&gt;</Link></div><div className="monitoring-list">{data.monitoring.map((item) => <article key={item.id}><div><span>{item.primary_domain?.replaceAll("_", " ") ?? "Market development"}{item.event_type ? ` · ${item.event_type.replaceAll("_", " ")}` : ""}{item.primary_entity ? ` · ${item.primary_entity}` : ""}</span><h3>{item.signal_id ? <Link href={`/signals/${item.signal_id}`}>{item.display_title}</Link> : item.display_title}</h3><p>{item.relevance_trace?.why_relevant || (item.matched_company_objects.length ? `Relevant to ${item.matched_company_objects.slice(0, 2).join(" and ")}` : item.what_changed)}</p><small>{item.evidence_available ? `${item.source_name} · Evidence available` : "Evidence is still being verified"}</small></div><time dateTime={item.published_at ?? item.detected_at}>{relativeTime(item.published_at ?? item.detected_at)}</time>{item.evidence_available && item.source_url && <a href={item.source_url} rel="noreferrer" target="_blank">Open evidence</a>}</article>)}</div>{!data.monitoring.length && <p className="quiet-state">No additional verified recent developments match your configured scope.</p>}</section>
    </div><aside className="briefing-rail">
      <section className="since-visit">
        <p className="eyebrow">Since your last visit</p>
        <strong>
          {data.changes.since_known === false
            ? "Your first briefing visit"
            : `${data.changes.new ?? (data.changes.new_briefs + data.changes.new_relevant_monitoring)} new developments`}
        </strong>
        <div style={{ display: "grid", gap: "4px", fontSize: "0.76rem", color: "var(--text-secondary)", marginTop: "8px" }}>
          <span>• {data.changes.new_decision ?? data.changes.new_briefs} new decisions</span>
          <span>• {data.changes.updated ?? data.changes.updated_briefs} assessments updated</span>
          <span>• {data.changes.escalated ?? 0} escalated posture</span>
          <span>• {data.changes.new_evidence ?? data.changes.new_evidence_items} new evidence sources</span>
        </div>
      </section>
      <section className="focus-panel">
        <p className="eyebrow">Monitoring Freshness</p>
        <div style={{ display: "grid", gap: "6px", fontSize: "0.78rem", color: "var(--text-secondary)", marginBottom: "12px" }}>
          {data.readiness.last_checked_at && (
            <p style={{ margin: 0 }}>
              <strong>System check:</strong> {relativeTime(data.readiness.last_checked_at)}
            </p>
          )}
          {data.changes.content_freshness_at && (
            <p style={{ margin: 0 }}>
              <strong>Latest verified development:</strong> {relativeTime(data.changes.content_freshness_at)}
            </p>
          )}
        </div>
        <h2>Focus Area activity</h2>
        <p>Your Company Context and personal Focus Areas shape ranking without changing the underlying evidence.</p>
        <Link className="text-link" href="/company">Review Company Scope</Link>
      </section>
    </aside></section>}
  </WorkspaceShell>;
}
