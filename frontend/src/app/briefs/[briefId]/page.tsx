...
export default function BriefDetailPage() {
  ...
  return <WorkspaceShell><section className="detail-page">
    ...
    {state.status === "ready" && <div className="decision-brief-grid"><article className="brief-detail brief-detail-v2">
      <header><div className="brief-meta"><span className={`priority-chip priority-${state.data.relevance_band.toLowerCase()}`}>{state.data.relevance_band}</span><span>{readable(state.data.primary_domain)}</span>{Number(state.data.material_change_count ?? 0) > 0 && <span className="updated-chip">Updated</span>}</div><h1>{state.data.what_changed}</h1><p>{state.data.published_at ? <>Source published <time dateTime={state.data.published_at}>{new Date(state.data.published_at).toLocaleString()}</time></> : "Source publication date unconfirmed"}{state.data.last_material_change_at && <> ; Brief updated <time dateTime={state.data.last_material_change_at}>{new Date(state.data.last_material_change_at).toLocaleString()}</time></>}</p><p className="brief-deck">{state.data.why_it_matters || "Stem is retaining this development for a bounded decision review."}</p></header>
      ...
      <section className="split-evidence"><div><p className="section-kicker">03</p><h2>What&apos;s affected</h2><p>{state.data.exposure_summary || "Exposure is still being assessed."}</p><div className="brief-chip-row">{state.data.exposure_types?.map((item) => <span key={item}>{readable(item)}</span>)}</div></div><div><p className="section-kicker">04</p><h2>What&apos;s at stake</h2><p>{state.data.stakes_summary || "No quantified stake has been verified."}</p><div className="brief-chip-row">{state.data.stakes_types?.map((item) => <span className="stakes-chip" key={item}>{readable(item)}</span>)}</div></div></section>
      ...
      <section><h2>Uncertainty</h2>{state.data.uncertainties.length ? <ul>{state.data.uncertainties.map((item) => <li key={item}>{readable(item)}</li>)}</ul> : <p>No additional uncertainty was recorded.</p>}<p>{readable(state.data.quantification_status)}</p></section>
      ...
      {state.data.timeline?.length ? <section><p className="section-kicker">08</p><h2>Timeline</h2><ol className="brief-timeline">{state.data.timeline.map((event, index) => <li key={`${event.event_type}-${index}`}><span /><div><strong>{readable(event.event_type)}</strong><time dateTime={event.created_at}>{new Date(event.created_at).toLocaleString()}</time></div></li>)}</ol></section> : null}
    </article><aside className="brief-context-rail"><section><p className="eyebrow">Decision rail</p><dl><div><dt>Materiality</dt><dd>{readable(state.data.relevance_band)} company relevance</dd></div><div><dt>Status</dt><dd>{readable(state.data.brief_status)}</dd></div><div><dt>Owner</dt><dd>{state.data.owner_roles?.join(", ") || "Leadership"}</dd></div><div><dt>Confidence</dt><dd>{readable(state.data.confidence_band) || "Reviewed"}</dd></div><div><dt>Decision window</dt><dd>{state.data.decision_window || "Monitor"}</dd></div><div><dt>Context matches</dt><dd>{state.data.matched_company_objects?.join(", ") || "No named context matches are available."}</dd></div><div><dt>Why shown</dt><dd>{state.data.personal_priority_score ? `${Math.round(Number(state.data.personal_priority_score) * 100)}% Decision Lens match` : "Company priority"}</dd></div></dl></section><button className="primary-button investigate-button" onClick={() => { setShowCil(true); void recordProductEvent("CIL_OPENED", { object_type: "DECISION_BRIEF", object_id: id }); }} type="button">Investigate with Cogent</button><div className="rail-actions"><h2>Actions</h2>{actions.map(([value, label]) => <button disabled={pendingAction !== null} key={value} onClick={() => void act(value)} type="button">{pendingAction === value ? "Saving..." : label}</button>)}</div>{actionMessage && <p aria-live="polite" className="form-message">{actionMessage}</p>}{showCil && <CILPanel anchorId={id} />}</aside></div>}
