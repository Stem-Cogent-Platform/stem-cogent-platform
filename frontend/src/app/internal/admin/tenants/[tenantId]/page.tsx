...
type Detail = { tenant: Row; readiness?: { ready: boolean; reason: string; company_briefs: number; meaningful_monitoring_count: number; pending_context_references?: number }; checklist: Record<string, boolean>; company_objects: Row[]; users: Row[]; invitations: Row[]; activations: Row[]; briefs: Row[] };
type Metrics = Row;
...
function readinessMessage(readiness: Detail["readiness"]) {
  if (readiness?.ready) return "First Value is ready. You can create an invitation.";
  if (readiness?.reason === "NOT_READY_CONTEXT_INCOMPLETE") return "Add the company products, markets and priorities before activation.";
  if (readiness?.reason === "NOT_READY_ACTIVATION_INCOMPLETE") return "Run activation for the current Company Context.";
  return "Stem needs at least one current Decision Brief or three meaningful monitoring developments before invitations are available.";
}

function DataRows({ rows, empty }: { rows: Row[]; empty: string }) {
  if (!rows.length) return <p className="quiet-state">{empty}</p>;
  return <div className="admin-data-rows">{rows.map((row, index) => <article key={String(row.id ?? index)}>{Object.entries(row).filter(([, value]) => value !== null).map(([key, value]) => <div key={key}><small>{key.replaceAll("_", " ")}</small><span>{typeof value === "object" ? JSON.stringify(value) : String(value)}</span></div>)}</article>)}</div>;
}
...
export default function TenantDetailPage() {
  ...
  const activationRunning = state.status === "ready" && state.data.detail.activations.some(run => ["QUEUED", "RUNNING"].includes(String(run.status)));
  useEffect(() => { if (!activationRunning) return; const timer = setInterval(() => void load(), 3000); return () => clearInterval(timer); }, [activationRunning, load]);
  async function operation(name: string, path: string, init?: RequestInit) { setPending(name); setMessage(""); try { const result = await apiRequest<Record<string, unknown>>(path, init); setMessage(name === "Invite" && result.invitation_url ? `Single-use invitation: ${result.invitation_url}` : name === "Activation" ? "Activation started. Readiness will update when processing finishes." : `${name} completed.`); await load(); } catch { setMessage(`${name} could not be completed.`); } finally { setPending(""); } }
  ...
  return <InternalAdminShell><header className="internal-heading"><div><p className="eyebrow">Pilot tenant</p><h1>{String(detail.tenant.name)}</h1></div><span className="admin-status">{String(detail.tenant.status)}</span></header><div className="admin-tabs" role="tablist">{tabs.map((item) => <button aria-selected={tab === item} className={tab === item ? "active" : ""} key={item} onClick={() => setTab(item)} role="tab" type="button">{item}</button>)}</div>{message && <p className="form-message" role="status">{message}</p>}<section className="internal-card tenant-tab">
    {tab === "Overview" && <><h2>Readiness checklist</h2><p role="status">{detail.readiness?.ready ? "Ready to invite" : "Not ready to invite"} ? {detail.readiness?.reason.replaceAll("_", " ")}</p><p>{detail.readiness?.company_briefs ?? 0} current company briefs ? {detail.readiness?.meaningful_monitoring_count ?? 0} meaningful monitoring items</p><div className="readiness-grid">{Object.entries(detail.checklist).map(([key, ready]) => <div className={ready ? "ready" : "pending"} key={key}><span>{ready ? "✓" : "○"}</span>{key.replaceAll("_", " ")}</div>)}</div></>}
