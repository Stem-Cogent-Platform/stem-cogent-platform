...
type Activity = { relevant_development_count?: number; new_count?: number; latest_title?: string; latest_signal_id?: string; latest_activity_at?: string; monitoring_state?: string };
type WatchItem = Activity & { id: string; name: string; object_type: string; importance: string; entity_id?: string; recent_activity_count: number | null; open_brief_count: number };
type FocusItem = Activity & { id: string; label: string; focus_type: string; weight?: number; entity_id?: string; recent_activity_count: number | null; open_brief_count: number | null };
type Watchlist = { company: WatchItem[]; focus: FocusItem[] };
const tabs = ["COMPETITOR", "PRODUCT", "DEPENDENCY", "MARKET", "REGULATORY_CATEGORY", "FOCUS"] as const;
const tabLabels = { COMPETITOR: "Competitors", PRODUCT: "Products", DEPENDENCY: "Dependencies", MARKET: "Markets", REGULATORY_CATEGORY: "Regulators", FOCUS: "My Focus Areas" };
...
export default function WatchlistPage() {
  ...
  return <WorkspaceShell><section className="content-page"><div className="page-heading"><div><p className="eyebrow">Monitoring scope</p><h1>Watchlist & Focus Areas</h1></div><Link className="secondary-button" href="/onboarding/lens">Add a Focus Area</Link></div>
    ...
    {state.status === "ready" && <section className="panel watchlist-tabs"><div aria-label="Watchlist category" className="tab-strip" role="tablist">{tabs.map((value) => <button aria-selected={tab === value} className={tab === value ? "active" : ""} key={value} onClick={() => setTab(value)} role="tab" type="button">{tabLabels[value]}</button>)}</div><div className="watchlist-rows">{items.map((raw) => { const isFocus = "label" in raw; const title = isFocus ? raw.label : raw.name; const type = isFocus ? "FOCUS AREA" : raw.object_type; return <article className="watch-item-v2" key={raw.id}><div><small>{type.replaceAll("_", " ")}</small><h2>{title}</h2><p>{raw.recent_activity_count ? `${raw.recent_activity_count} relevant development${raw.recent_activity_count === 1 ? "" : "s"}` : "Monitoring ? No verified developments matched in the last 30 days"}{raw.open_brief_count ? ` · ${raw.open_brief_count} requiring attention` : ""}</p>{raw.new_count ? <p>{raw.new_count} new in the last 24 hours</p> : null}{raw.latest_signal_id && <p>Latest: <Link href={`/signals/${raw.latest_signal_id}`}>{raw.latest_title}</Link>{raw.latest_activity_at && <small> ? {new Date(raw.latest_activity_at).toLocaleString()}</small>}</p>}</div>{raw.entity_id ? <Link href={`/entities/${raw.entity_id}`} onClick={() => void recordProductEvent("WATCHLIST_ITEM_VIEWED", { object_type: type, object_id: raw.id })}>View</Link> : isFocus ? <Link href="/settings">Edit</Link> : <span>Company Context</span>}</article>; })}{!items.length && <div className="quiet-state"><p>{tab === "FOCUS" ? stateMessages.focusEmpty : stateMessages.watchlistEmpty}</p></div>}</div></section>}
