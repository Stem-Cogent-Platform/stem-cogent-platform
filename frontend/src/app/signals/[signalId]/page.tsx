  ...
  return <WorkspaceShell><section className="content-page">
    ...
    {state.status === "ready" && <>
      ...
      <div className="detail-grid"><article className="brief-detail">
        <p className="eyebrow">{state.data.signal.freshness === "HISTORICAL" ? "Historical context" : state.data.signal.freshness === "DATE_UNCERTAIN" ? "Publication date unconfirmed" : state.data.signal.freshness?.toLowerCase()}</p><p>{state.data.signal.subcategory_tags?.[0]?.replaceAll("_", " ")}</p><h2>Global facts: What changed</h2>
        ...
        <h2>Why this matters to your company</h2>
        {state.data.tenant_interpretation ? <section>
          <p>Company interpretation: {state.data.tenant_interpretation.relevance_band.toLowerCase()} relevance</p>
          <p>{state.data.tenant_interpretation.matched_company_objects.length ? `Matches your configured ${state.data.tenant_interpretation.matched_company_objects.join(", ")}.` : "Matches the company's configured decision criteria."}</p>
        ...
        {state.data.related_intelligence?.filter((item) => item.freshness !== "HISTORICAL").length ? <ul>{state.data.related_intelligence.filter((item) => item.freshness !== "HISTORICAL").map((item) => <li key={item.id}><Link href={`/signals/${item.id}`}>{item.title}</Link></li>)}</ul> : <p>No additional related developments are stored.</p>}
