...
export function CILPanel({ anchorId, anchorType = "DECISION_BRIEF", hasRelationships = false }: { anchorId: string; anchorType?: "DECISION_BRIEF" | "SIGNAL" | "ENTITY" | "COMPANY_LENS"; hasRelationships?: boolean }) {
  ...
  return (
    <>
    <button className="cil-mobile-trigger primary-button" onClick={() => setMobileOpen(true)} type="button">Investigate with Cogent</button>
    <aside aria-label="Cogent investigation" className={mobileOpen ? "cil-panel mobile-open" : "cil-panel"}>
      <button aria-label="Close Cogent investigation" className="cil-mobile-close" onClick={() => setMobileOpen(false)} type="button">×</button>
