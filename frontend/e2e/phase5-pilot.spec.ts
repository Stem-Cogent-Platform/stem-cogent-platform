...
async function authenticatedApi(page: Page, permissionRole = "ADMIN") {
  ...
  await page.route("**/api/v1/capabilities", (route) => route.fulfill({ json: { phase5_brief_lifecycle_enabled: true, phase5_new_ui_enabled: true } }));
...
test("renders all pilot product surfaces at every supported width without overflow", async ({ page }, testInfo) => {
