        ...
        await context.session.execute(
            text(
                ...
                SELECT plan_code, name, monthly_price_cents, currency, trial_days,
                       entitlements, provider_plan_code
...
async def initialize_checkout(
    ...
    if not plan["provider_plan_code"]:
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE,
            detail={"code": "BILLING_PLAN_SYNC_PENDING",
                    "message": "Checkout is being prepared. Please try again shortly."},
        )
    user = (
        await context.session.execute(
    ...
    if existing and existing["status"] == "INITIALIZED" and existing["authorization_url"]:
    ...
    await context.session.execute(
        ...
        {"tenant_id": context.principal.tenant_id, "user_id": context.principal.user_id,
         ...
         "amount": plan["monthly_price_cents"], "currency": plan["currency"],
         "expires_at": expires_at},
    )
    await context.session.commit()
    try:
        checkout = await PaystackClient(_paystack_secret()).initialize_transaction(
            {"email": user["email"], "amount": str(plan["monthly_price_cents"]),
             ...
             "metadata": json.dumps({"tenant_id": str(context.principal.tenant_id),
                                      ...
                                      "plan_code": plan["plan_code"]}, separators=(",", ":"))}
        )
...
async def _activate_checkout(session: Any, tenant_id: UUID, reference: str, data: dict[str, Any]) -> None:
    ...
    await session.execute(
        text("UPDATE auth.tenants SET plan_tier = :plan_code, status = 'ACTIVE', updated_at = NOW() WHERE id = :tenant_id"),
        {"tenant_id": tenant_id, "plan_code": intent["plan_code"]},
    )
    await session.execute(
