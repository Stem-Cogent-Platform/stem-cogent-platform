...
variable "public_hosted_zone_name" {
  description = "Existing public Route 53 zone used for staging aliases and ACM DNS validation."
  type        = string
  default     = "stem-cogent.com"
}

variable "frontend_public_url" {
  description = "Canonical public staging frontend origin."
  type        = string
...
variable "alb_deletion_protection" {
  description = "Whether the staging ALB rejects deletion through the AWS API."
  type        = bool
  default     = false
}

variable "ecs_bootstrap_image_tag" {
  description = "Full immutable commit SHA pushed to all staging ECR repositories by the Task 1.3.12 build-only run."
  type        = string
